  <title>Universidade da Vida - INSEJEC SJC</title>
  <link rel="stylesheet" href="css/all.min.css">
  <link rel="stylesheet" href="css/uv.css">

    <section class="inicial centro">
      <div class="centro container" >
        <a target="_blank" onclick="registro()" href="https://contribua.prover.app?pc=VALXH9unkfS-kLJEsk68aa7aIm7yp1FN7apAue82Fwo,u=3369cfd81942d6a1e027b2d46f49dcb8">REGISTRAR-SE</a>
        </div>
    </section>
  
    <section class="descricao">
      <div class="centro">
        <div class="flex">
          <div class="w50">
            <img src="./img/uv/bg2UV.png" alt="">
          </div>
          
          <div class="w50">
            <h2>Aulas totalmente on-line!</h2>
            <h2>1x de R$29,90</h2>
            <h2>Domingos das 8h00 às 9h00</h2>
            <details>
              <summary>Datas</summary>
              <section>
                <ul>
                  <li>Semana 1 - 18/10</li>
                  <li>Semana 2 - 25/10</li>
                  <li>Semana 3 - 01/11</li>
                  <li>Semana 4 - 08/11</li>
                  <li>Semana 5 - Encontro - 12 ~ 14/11</li>
                  <li>Semana 6 - 22/11</li>
                  <li>Semana 7 - 29/11</li>
                  <li>Semana 8 - 06/12</li>
                  <li>Semana 9 - 13/12</li>
                </ul>
              </section>
            </details>
            
            <details>
              <summary>Informações sobre o curso</summary>
              <section>
                <ul>
                  <li><strong>9</strong>​ semanas de aulas</li>
                  <li> <strong>63</strong>  exercícios no total</li>
                  <li>  <strong>25</strong> minutos diários</li>
                  <li> <strong>1</strong> aula semanal em grupo</li>
                </ul>
              </section>
            </details>
            <div id="botaoRegistro">
              <a target="_blank" onclick="registro()" href="https://contribua.prover.app?pc=VALXH9unkfS-kLJEsk68aa7aIm7yp1FN7apAue82Fwo,u=3369cfd81942d6a1e027b2d46f49dcb8">REGISTRAR-SE</a>
            </div>
            

          </div>
        </div>
      </div>
    </section>

    <section class="textos">
      <div class="centro">
        <h1>Por que fazer a Universidade da Vida ?</h1>
        <div class="flex">
          <div class="w33">
            <div class="bolinha" id="bolinha1" style="background-image: url('<?php echo INCLUDE_PATH ?>img/uv/uv-ico-viver.png');"></div>
            <h2>Te ensina a viver</h2>
            <p>Cada semana você aprenderá princípios práticos e simples para viver. Você dará um passo de cada vez, sem correr! Cada passo o deixará mais perto de viver uma vida como você nunca imaginou com Deus.</p>
          </div>
          <div class="w33">
            <div class="bolinha" id="bolinha2" style="background-image: url('<?php echo INCLUDE_PATH ?>img/uv/uv-ico-sozinho.png');"></div>
            <h2>Você não está sozinho</h2>
            <p>Todos nós temos perguntas como: O que é fé? Quem é Jesus? Como orar? Haverá algo de bom no meio de minhas falhas? Eu ainda tenho esperança? Por que devo ir à igreja?
              Em cada aula você terá espaço para fazer essas perguntas e encontrar a resposta.</p>
          </div>
          <div class="w33">
            <div class="bolinha" id="bolinha3" style="background-image: url('<?php echo INCLUDE_PATH ?>img/uv/uv-ico-mesmo.png');"></div>
            <h2>Você não será o mesmo</h2>
            <p>Depois de iniciar este curso de 9 semanas, você terá a oportunidade de mudar o curso de sua vida. É simples! Por meio da Cruz, Jesus oferece a você deixar seu passado para trás e começar uma nova história de vida.</p>
          </div>
        </div>


      </div>
    </section>

    <section class="cronograma">
      <div class="centro">
      <br><br><br>
        <h1>Cronograma das aulas</h1>
        <br><br><br><br><br>
        <a class="ctaa" style="background-image: url(./img/uv/1-uv-mod-erros.png);"></a>
        <a class="ctaa" style="background-image: url(./img/uv/2-uv-mod-negociacao.png);"></a>
        <a class="ctaa" style="background-image: url(./img/uv/3-uv-mod-experiencia.png);"></a>
        <br><br><br><br>
        <a class="ctaa" style="background-image: url(./img/uv/4-uv-mod-batalha.png);"></a>
        <a class="ctaa" style="background-image: url(./img/uv/5-uv-mod-encontro.png);"></a>
        <a class="ctaa" style="background-image: url(./img/uv/6-uv-mod-segredo.png);"></a>
        <br><br><br><br>
        <a class="ctaa" style="background-image: url(./img/uv/7-uv-mod-decisoes.png);"></a>
        <a class="ctaa" style="background-image: url(./img/uv/8-uv-mod-melhor.png);"></a>
        <a class="ctaa" style="background-image: url(./img/uv/9-uv-mod-comeco.png);"></a>
        <br><br><br><br>
      </div>
    </section>
    
    <section class="duvidas">
      <div class="centro">
        <div class="brand"><h1>Dúvidas ? Fale com a gente :</h1></div>
          <div class="sociais">
            <div>
              <a id="face" href="https://www.facebook.com/insejecsjcampos" target="_blank" class="bolinhaMenor"><i class="fab fa-facebook"></i></a>
            </div>
            <div>
              <a id="insta" href="https://www.instagram.com/insejec_sjc/" target="_blank" class="bolinhaMenor"><i class="fab fa-instagram"></i></a>
            </div>
            <div>
              <a id="whats" href="https://wa.me/5512982810294?text=Olá,%20gostaria%20de%20saber%20mais%20sobre%20a%20Universidade%20da%20Vida!" target="_blank" class="bolinhaMenor"><i class="fab fa-whatsapp"></i></a>
            </div>
          </div>
      </div>
    </section>
  <script src="<?php echo INCLUDE_PATH ?>js/adBlock.js"></script>
  <script src="<?php echo INCLUDE_PATH ?>js/teste.js"></script>
  <script src="js/all.min.js"></script>